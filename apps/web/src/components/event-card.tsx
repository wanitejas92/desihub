  ...
  return (
    <Link href={`/e/${event.slug}`} className={cn('group block', className)}>
      <div className="card-media bg-bg-sunken relative isolate aspect-[4/5] rounded-md">
        ...
        {/* Date always; the time only where it fits. At 390px a two-up grid
            gives the chip about 120px before it runs under the favourite
            button, and "Sat, 12 Sept · 19:30" needs more than that — it was
            being sliced mid-digit. The date is the field people scan; the
            time is on the card's own page. */}
        <span className="absolute top-3 left-3 z-10 max-w-[calc(100%-3.5rem)] truncate rounded-sm bg-black/55 px-2 py-1 text-[11px] font-semibold text-white backdrop-blur-sm">
          {formatEventDateCompact(event.starts_at)}
          <span className="hidden sm:inline"> · {formatEventTime(event.starts_at)}</span>
        </span>
